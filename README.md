...
Приложение читает CSV-файлы из указанной директории. Каждый файл представляет определенный тип биржевых данных. Данные из всех файлов, прошедших фильтр `filename_mask`, объединяются в единый набор и сортируются по `receive_ts`. Медиана рассчитывается по полю `price` из всех файлов, а не из какого-то одного.
...
Приложение создает один CSV-файл с фиксированным именем `median_result.csv` в выходной директории. При повторном запуске файл перезаписывается.
...
- `filename_mask` (опциональный) — список строк для фильтрации имен файлов. Фильтрация работает как **поиск подстроки**: читаются только те файлы, имена которых содержат хотя бы одну из указанных масок. Например, маска `'level'` соответствует файлу `btcusdt_level_2024.csv`, так как имя файла содержит подстроку `level`. Если список пустой, читаются все CSV-файлы
...
3. **Сканирование файловой системы**: поиск CSV-файлов в указанной директории с применением фильтров из `filename_mask`. Найденные файлы **сортируются лексикографически по имени** перед дальнейшей обработкой
...
5. **Сортировка данных** по полю `receive_ts` с использованием `std::stable_sort` для сохранения относительного порядка записей с одинаковым `receive_ts`. Поскольку файлы объединяются в лексикографическом порядке, при равном `receive_ts` записи из файла с лексикографически меньшим именем будут идти первыми — это обеспечивает детерминированный и воспроизводимый результат
...
7. **Запись результатов** в выходной CSV-файл `median_result.csv`
...
3. **Подключение зависимостей:** `toml++` и `spdlog` подключаются через `FetchContent`. Boost является системной зависимостью и подключается через `find_package`:
   ```cmake
...
- Результаты сохраняются в файл `./results/median_result.csv`
...
- Результаты сохраняются в файл `/var/results/median_result.csv`
...
**Результат:**
- Обрабатываются файлы с подстрокой `level` или `trade` в имени (например, `btcusdt_level_2024.csv`, `btcusdt_trade_2024.csv`)
- Создается директория `output` в текущей директории
- Результаты сохраняются в `./output/median_result.csv`
...
**Выходной файл (`median_result.csv`):**
```csv
receive_ts;price_median
1000;100.00000000
2000;100.50000000
...
> Записи с `receive_ts = 2000` дублируются в выходном файле, так как каждая из них изменила медиану. При использовании `std::stable_sort` относительный порядок записей с одинаковым `receive_ts` сохраняется.
...
- ✅ Корректно формируется выходной файл `median_result.csv` с точностью 8 знаков после запятой
